...
- **Path traversal**: `AudioClipHandler.SafeClipPath()` validates clip names via `Path.GetFileName()` + `Path.GetFullPath()` prefix check. `YappTransfer` uses the same `GetFullPath()` + prefix check pattern for received filenames. `WebServer` uses `..` check + `Path.GetFullPath()` canonicalization.
...
- **Buffer caps**: AGWPE DataLen ≤ 65536, SMTP data ≤ 10MB, SMTP recipients ≤ 100, IMAP APPEND ≤ 10MB, Rigctld uses byte-at-a-time `ReadLineLimitedAsync(1024)` to bound memory before buffering (prevents unbounded `ReadLineAsync` allocation), CAT command buffer ≤ 1024 bytes, TlsHttpServer ≤ 100 concurrent connections with 8KB per-header-line limit, `RadioAudioManager` audio accumulator checks size *before* write (prevents OOM from single oversized frame), AudioClipHandler SaveAudioClip ≤ 10MB, WebAudioBridge audio frames ≤ 19200 bytes with per-client rate limiting (200 frames/sec).
- **Injection prevention**: All Linux subprocess calls (espeak-ng, pactl, pacat, parecord) use `ProcessStartInfo.ArgumentList` instead of string concatenation — prevents argument injection regardless of input content. HTTP response headers stripped of CRLF and null bytes. IMAP tags sanitized; `ParseSequenceSet` and `ParseUidSet` use `TryParse` (not `Parse`) to handle malformed input gracefully; UID computation handles `int.MinValue` hash to prevent `Math.Abs` overflow. espeak-ng voice names filtered to `[a-zA-Z0-9_\-+]`. `SelfUpdateDialog` validates release URL via `Uri.TryCreate()` with host (`github.com`) and path (`/dikei100/HTCommander-X/releases/`) validation. `LinuxVirtualAudioProvider.LoadModule()` validates module names against an allowlist (`module-null-sink`, `module-virtual-source`). `DataBroker` JSON deserialization validates stored type name matches requested `T` to prevent unsafe deserialization.
