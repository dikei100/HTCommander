...
### MCP authentication
When `ServerBindAll` is enabled, the MCP server requires a `Authorization: Bearer <token>` header on all POST requests. The token is auto-generated (24 random bytes, base64) on first run and stored as `McpApiToken` in DataBroker device 0. `McpDebugToolsEnabled` is **not** in the `SettingsWhitelist` — it can only be toggled from the Settings UI, preventing remote privilege escalation via `set_setting`.

### Input validation patterns
- **Path traversal**: `AudioClipHandler.SafeClipPath()` validates clip names via `Path.GetFileName()` + `Path.GetFullPath()` prefix check. `Yapp` sanitizes remote filenames. `WebServer` uses `..` check + `Path.GetFullPath()` canonicalization.
- **Protocol bounds**: All radio data structure constructors validate minimum message length. `Radio.RadioTransport_ReceivedData` rejects messages < 4 bytes and catches `ArgumentException`. Channel ID bounds-checked before array access.
- **Buffer caps**: AGWPE DataLen ≤ 65536, SMTP data ≤ 10MB, IMAP APPEND ≤ 10MB, Rigctld/CAT command lines ≤ 1024 bytes, TlsHttpServer ≤ 100 concurrent connections.
- **Injection prevention**: HTTP response headers stripped of CRLF. IMAP tags sanitized. espeak-ng voice names filtered to `[a-zA-Z0-9_\-+]`, text arguments stripped of shell metacharacters. `SelfUpdateDialog` validates release URL starts with `https://github.com/`.

### Error handling
Protocol error responses (SMTP, IMAP, MCP JSON-RPC) use generic messages — never expose `ex.Message` to clients.

### File permissions
On Linux, PFX certificate files and settings JSON are written with chmod 600 (owner-only read/write). Debug log writes to `~/.config/HTCommander/debug.log` instead of CWD.

### Web security
Both `index.html` and `mobile.html` include Content-Security-Policy meta tags. CORS uses reflect-origin (not wildcard) with `Vary: Origin`. All innerHTML assignments use `escapeHtml()`.
