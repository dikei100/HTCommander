...
/// Linux Whisper STT engine using whisper.cpp CLI subprocess.
///
/// Accumulates PCM audio during a voice segment, writes a temporary WAV file,
/// and runs whisper-cli for inference. Same subprocess pattern as
/// LinuxSpeechService (espeak-ng).
class LinuxWhisperEngine implements WhisperEngine {
  ...
  LinuxWhisperEngine(this._modelPath, this._language) {
    _resolveBinary();
  }

  /// Tries to find whisper-cli, whisper, or main on PATH.
  void _resolveBinary() {
    for (final name in ['whisper-cli', 'whisper', 'main']) {
      try {
        final result = Process.runSync('which', [name]);
        if (result.exitCode == 0) {
          _binaryPath = result.stdout.toString().trim();
          onDebugMessage?.call('Whisper binary found: $_binaryPath');
          return;
        }
      } catch (_) {}
    }
    onDebugMessage?.call('Whisper binary not found (tried whisper-cli, whisper, main)');
  }
